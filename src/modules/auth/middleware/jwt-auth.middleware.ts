...
export class JwtAuthMiddleware implements NestMiddleware {
  ...
  async use(req: FastifyRequest, res: FastifyReply, next: () => void) {
    ...
    if (!authHeader) {
      this.logger.warn(`[JWT] No Authorization header found for ${req.method} ${req.url}`);
      next();
      return;
    }

    if (!authHeader.startsWith('Bearer ')) {
      this.logger.warn(`[JWT] Authorization header doesn't start with 'Bearer ' for ${req.method} ${req.url}`);
      next();
      return;
    }

    const token = authHeader.substring(7);
    this.logger.log(`[JWT] Attempting to verify token (length: ${token.length}) for ${req.method} ${req.url}`);

    try {
      const payload = await this.jwtService.verifyAccessToken(token);
      (req as any).user = payload;
      this.logger.log(`[JWT] ✓ Verification successful for user: ${payload.sub}, user set on request: ${!!(req as any).user}`);
    } catch (error) {
