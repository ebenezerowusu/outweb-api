...
import { CreateOrderDto, CreateOrderTransactionDto } from './dto/create-order.dto';
import {
...
export class OrdersService {
   ...
   */
  async findAll(query: QueryOrdersDto): Promise<{ items: PublicOrder[]; continuationToken?: string }> {
    const {
      buyerId,
      sellerId,
      listingId,
    ...
    const { items, continuationToken: nextToken } = await this.cosmosService.queryItems<OrderDocument>(
      ORDERS_CONTAINER,
      querySpec,
      parameters,
      limit,
   ...
   * Get order by ID
   */
  async findOne(id: string, userId: string, hasAdminPermission: boolean): Promise<PublicOrder> {
    const order = await this.cosmosService.getItem<OrderDocument>(ORDERS_CONTAINER, id, id);
    ...
    if (!hasAdminPermission && order.buyerId !== userId && order.sellerId !== userId) {
      throw new ForbiddenException({ message: 'You do not have permission to view this order' });
    }
  ...
  async create(dto: CreateOrderDto, userId: string): Promise<PublicOrder> {
    ...
    const order: OrderDocument = {
      ...
      timeline: {
        createdAt: now,
        depositDueAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), // 7 days
    ...
    const created = await this.cosmosService.createItem<OrderDocument>(ORDERS_CONTAINER, order);
  ...
  async updateStatus(
    ...
    if (!hasAdminPermission && order.sellerId !== userId) {
      throw new ForbiddenException({ message: 'Only the seller or admin can update order status' });
    }
    ...
    const updated = await this.cosmosService.upsertItem<OrderDocument>(ORDERS_CONTAINER, order);
   ...
   * Cancel order
   */
  async cancel(id: string, dto: CancelOrderDto, userId: string): Promise<PublicOrder> {
    const order = await this.cosmosService.getItem<OrderDocument>(ORDERS_CONTAINER, id, id);
    ...
    if (order.buyerId !== userId && order.sellerId !== userId) {
      throw new ForbiddenException({ message: 'You do not have permission to cancel this order' });
    }

    if (order.status.state === 'completed') {
      throw new BadRequestException({ message: 'Cannot cancel a completed order' });
    }
    ...
    order.timeline.events.push({
      ...
      performedBy: userId,
      metadata: { issueRefund: dto.issueRefund, refundAmount: dto.refundAmount },
      occurredAt: now,
    });
   ...
   * Schedule inspection
   */
  async scheduleInspection(id: string, dto: ScheduleInspectionDto, userId: string): Promise<PublicOrder> {
    const order = await this.cosmosService.getItem<OrderDocument>(ORDERS_CONTAINER, id, id);
    ...
    order.timeline.events.push({
      ...
      performedBy: userId,
      metadata: { scheduledAt: dto.scheduledAt, location: dto.location, inspector: dto.inspector },
      occurredAt: now,
    });
   ...
   * Complete inspection
   */
  async completeInspection(id: string, dto: CompleteInspectionDto, userId: string): Promise<PublicOrder> {
    const order = await this.cosmosService.getItem<OrderDocument>(ORDERS_CONTAINER, id, id);
    ...
    const eventType: OrderEventType = dto.approved ? 'inspection_approved' : 'inspection_rejected';
    ...
    order.timeline.events.push({
      ...
      performedBy: userId,
      metadata: { approved: dto.approved, findings: dto.findings, reportUrl: dto.reportUrl },
      occurredAt: now,
    });
   ...
   */
  async updateDelivery(id: string, dto: UpdateDeliveryDto, userId: string): Promise<PublicOrder> {
    const order = await this.cosmosService.getItem<OrderDocument>(ORDERS_CONTAINER, id, id);
    ...
    if (dto.estimatedArrival) order.delivery.estimatedArrival = dto.estimatedArrival;
    if (dto.trackingNumber) order.delivery.trackingNumber = dto.trackingNumber;
    if (dto.carrier) order.delivery.carrier = dto.carrier;
    if (dto.specialInstructions !== undefined) order.delivery.specialInstructions = dto.specialInstructions;
   ...
   * Add note to order
   */
  async addNote(id: string, dto: AddOrderNoteDto, userId: string, authorName: string): Promise<PublicOrder> {
    const order = await this.cosmosService.getItem<OrderDocument>(ORDERS_CONTAINER, id, id);
   ...
   */
  async addDocument(id: string, dto: AddOrderDocumentDto, userId: string): Promise<PublicOrder> {
    const order = await this.cosmosService.getItem<OrderDocument>(ORDERS_CONTAINER, id, id);
  ...
  async findAllTransactions(
    ...
    const { items, continuationToken: nextToken } = await this.cosmosService.queryItems<OrderTransactionDocument>(
      ORDERS_CONTAINER,
      querySpec,
      parameters,
      limit,
   ...
   * Create transaction
   */
  async createTransaction(dto: CreateOrderTransactionDto, userId: string): Promise<PublicTransaction> {
    const order = await this.cosmosService.getItem<OrderDocument>(ORDERS_CONTAINER, dto.orderId, dto.orderId);
    ...
    const created = await this.cosmosService.createItem<OrderTransactionDocument>(ORDERS_CONTAINER, transaction);
    return created;
  }
   ...
   */
  private updateTimelineForState(order: OrderDocument, state: OrderState, now: string): void {
    switch (state) {
      case 'deposit_paid':
