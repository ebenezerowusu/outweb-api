...
export class NotificationsService {
  ...
  async findAll(
    ...
    const { items, continuationToken: nextToken } =
      await this.cosmosService.queryItems<NotificationDocument>(
  ...
  async findOne(id: string, userId: string): Promise<PublicNotification> {
    const notification = await this.cosmosService.getItem<NotificationDocument>(
      NOTIFICATIONS_CONTAINER,
      id,
      userId,
    );
  ...
  async create(dto: CreateNotificationDto): Promise<PublicNotification> {
    ...
    const created = await this.cosmosService.createItem<NotificationDocument>(
      NOTIFICATIONS_CONTAINER,
      notification,
    );
  ...
  async markAsRead(
    ...
    const updated = await this.cosmosService.upsertItem<NotificationDocument>(
      NOTIFICATIONS_CONTAINER,
      notification,
    );
    return updated;
  ...
  async markAllAsRead(userId: string): Promise<{ updated: number }> {
    ...
    const querySpec =
      "SELECT * FROM c WHERE c.type = 'notification' AND c.userId = @userId AND c.status.read = false";
    ...
    const updatePromises = notificationItems.map((notification) => {
      ...
      notification.audit.updatedAt = now;
      return this.cosmosService.upsertItem<NotificationDocument>(
        NOTIFICATIONS_CONTAINER,
        notification,
      );
    });
  ...
  async updatePreferences(
    ...
    const updated =
      await this.cosmosService.upsertItem<NotificationPreferencesDocument>(
        NOTIFICATIONS_CONTAINER,
        preferences,
      );
    return updated;
  ...
  private async getOrCreatePreferences(
    ...
    let preferences =
      await this.cosmosService.getItem<NotificationPreferencesDocument>(
        NOTIFICATIONS_CONTAINER,
        prefId,
        userId,
      );
    ...
    if (!preferences) {
      ...
      preferences =
        await this.cosmosService.createItem<NotificationPreferencesDocument>(
          NOTIFICATIONS_CONTAINER,
          preferences,
        );
    }
  ...
  private getEnabledChannels(
    ...
    if (
      ...
      notificationType.includes("payment_") ||
      notificationType.includes("inspection_") ||
      notificationType.includes("delivery_")
    ) {
      category = "orders";
    } else if (
      notificationType.startsWith("listing_") ||
      notificationType.includes("offer_")
    ) {
      category = "listings";
    } else if (
      notificationType.startsWith("subscription_") ||
      notificationType.includes("invoice_")
    ) {
      category = "subscriptions";
    } else if (notificationType.includes("review_")) {
      category = "reviews";
    } else if (
      notificationType.includes("message_") ||
      notificationType.includes("chat_")
    ) {
      category = "chats";
    }
    ...
    return requestedChannels.filter((channel) => {
      ...
      const categoryEnabled =
        categoryPrefs[channel as keyof typeof categoryPrefs];
  ...
  private async sendNotificationChannels(
    ...
    notification.audit.updatedAt = now;
    await this.cosmosService.upsertItem<NotificationDocument>(
      NOTIFICATIONS_CONTAINER,
      notification,
    );
  }
