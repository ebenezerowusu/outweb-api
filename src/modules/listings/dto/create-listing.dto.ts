...
export class CreateListingVehicleDto {
  @ApiProperty({ description: 'VIN (17 characters)', example: '5YJ3E1EA1KF123456' })
  @IsString()
  @Matches(/^[A-HJ-NPR-Z0-9]{17}$/, { message: 'VIN must be 17 alphanumeric characters' })
  vin: string;

  @ApiProperty({ description: 'Vehicle make taxonomy ID', example: 'tax_make_tesla' })
  @IsString()
  makeId: string;

  @ApiProperty({ description: 'Vehicle model taxonomy ID', example: 'tax_model_model3' })
  @IsString()
  modelId: string;
  ...
  @ApiProperty({ description: 'Manufacturing year', example: 2021, minimum: 2008, maximum: 2030 })
  @IsInt()
  @Min(2008)
  @Max(2030)
  year: number;
  ...
  @ApiProperty({ description: 'Battery health percentage (0-100)', required: false })
  @IsInt()
  @Min(0)
  @Max(100)
  @IsOptional()
...
export class CreateListingConditionDto {
  ...
  @ApiProperty({ description: 'Exterior rating (1-10)', minimum: 1, maximum: 10 })
  @IsInt()
  @Min(1)
  @Max(10)
  exteriorRating: number;

  @ApiProperty({ description: 'Interior rating (1-10)', minimum: 1, maximum: 10 })
  @IsInt()
  @Min(1)
  @Max(10)
  interiorRating: number;

  @ApiProperty({ description: 'Mechanical rating (1-10)', minimum: 1, maximum: 10 })
  @IsInt()
  @Min(1)
  @Max(10)
  mechanicalRating: number;

  @ApiProperty({ description: 'Condition description', example: 'Excellent condition, well maintained' })
  @IsString()
  @MinLength(20)
  @MaxLength(2000)
  description: string;
...
export class CreateListingDto {
  @ApiProperty({ description: 'Seller ID (optional, defaults to current user)' })
  @IsString()
  @IsOptional()
  sellerId?: string;
  ...
  @ApiProperty({ description: 'Standard features (taxonomy IDs)', type: [String], required: false })
  @IsArray()
  @IsString({ each: true })
  @IsOptional()
  standardFeatures?: string[];

  @ApiProperty({ description: 'Optional features (taxonomy IDs)', type: [String], required: false })
  @IsArray()
  @IsString({ each: true })
  @IsOptional()
