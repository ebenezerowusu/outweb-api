...
export class RbacService {
  ...
  async findAllRoles(
    query: QueryRolesDto,
  ): Promise<PaginatedResponse<PublicRole>> {
    ...
    const { items, continuationToken } =
      await this.cosmosService.queryItems<RoleDocument>(
  ...
  async getRolePermissions(roleId: string): Promise<{
    ...
    const parameters = permissionKeys.map((key, i) => ({
      name: `@id${i}`,
      value: key,
    }));
  ...
  async attachPermissions(
    ...
    const parameters = dto.permissionIds.map((id, i) => ({
      name: `@id${i}`,
      value: id,
    }));

    const { items: permissions } =
      await this.cosmosService.queryItems<PermissionDocument>(
        this.PERMISSIONS_CONTAINER,
        sqlQuery,
        parameters,
      );
  ...
  ): Promise<PaginatedResponse<PublicPermission>> {
  ...
  async deletePermission(permissionId: string): Promise<void> {
    ...
    const rolesUsingPermission =
      await this.findRolesUsingPermission(permissionId);
  ...
  async getPermissionCategories(): Promise<{ data: string[] }> {
    const sqlQuery =
      'SELECT DISTINCT c.category FROM c WHERE c.type = "permission"';
  ...
  private async validatePermissions(permissionIds: string[]): Promise<void> {
    ...
    const parameters = permissionIds.map((id, i) => ({
      name: `@id${i}`,
      value: id,
    }));
   ...
   */
  private async findRolesUsingPermission(
    permissionId: string,
