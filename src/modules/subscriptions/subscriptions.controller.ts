...
export class SubscriptionsController {
  ...
  async getMySubscription(@CurrentUser() user: any) {
    ...
    const result = await this.subscriptionsService.findAll({ userId: user.sub, limit: 1 });
  ...
  async getInvoices(
    ...
    const hasAdminPermission = user.permissions?.includes('perm_manage_users');
    return this.subscriptionsService.getInvoices(id, query, user.sub, hasAdminPermission);
  }
